...
export async function updateUser(req, res) { // nhớ login trước, đặt ở Header trường Key: Authorization và Value: Bearer <auth_token> xong có thể update email, pw, name
...
export async function deleteUser(req, res) {
  try {
      ...
      if (loggedInUser.role === "admin") {
          ...
          return res.status(200).json({ message: `User ${userIdToDelete} deleted successfully` });
      } else if (loggedInUser.role === "customer") { // chỉ có 2 role 
          ...
          if (userIdToDelete && userIdToDelete !== String(customerId)) {
              throw new HttpException(403, "Customers can only delete their own account");
          }
